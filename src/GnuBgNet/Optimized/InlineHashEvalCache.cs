...
// EvalCache variant with inline MurmurHash3 — no stackalloc intermediary.
...
/// <summary>
/// Drop-in replacement for <see cref="EvalCache"/> with inline hash computation.
/// Removes the stackalloc uint[7] span from GetHashKey by operating directly on
/// PositionKey fields. Same 2-way set-associative structure, not thread-safe.
/// </summary>
public sealed class InlineHashEvalCache : IEvalCache
{
    ...
    public InlineHashEvalCache(int sizeLog2)
    {
    ...
    {
        uint l = GetHashInline(key, evalContext);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private uint GetHashInline(PositionKey key, int evalContext)
    {
