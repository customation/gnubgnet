...
{
    private const int CacheLog2 = 16; // 65K entries — small enough for benchmark
    private EvalCache _original = null!;
    private ConcurrentEvalCache _concurrent = null!;
    ...
    public void Setup()
    {
        ...
        _concurrent = new ConcurrentEvalCache(CacheLog2);

        // Generate distinct keys from varied board positions
        ...
        {
            var board = new Board();
            // Distribute 15 checkers randomly for each side
            int remaining = 15;
    ...
    [Benchmark]
    public void Concurrent_AddAndLookup()
    {
        Span<float> output = stackalloc float[Constants.NumOutputs];
        for (int i = 0; i < Operations; i++)
        {
            uint l = _concurrent.Lookup(_keys[i], 0, output);
            if (l != EvalCache.CacheHit)
                _concurrent.Add(_keys[i], 0, _values[i], l);
        }
        // Second pass: all should be hits
        for (int i = 0; i < Operations; i++)
            _concurrent.Lookup(_keys[i], 0, output);
    }
}
